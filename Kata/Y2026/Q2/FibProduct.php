...
function productFib(int $requestedNumber): array
{
    $now = 1;
    $previous = 0;
    do {
        if ($now * $previous === $requestedNumber) {
            return [$previous, $now, true];
        }
        $previousResult = $previous + $now;

        $previous = $now;
        $now = $previousResult;
    } while ($now * $previous <= $requestedNumber);

    return [$previous, $now, false];
}
